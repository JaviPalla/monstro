...
**Variant (España / México / Ambas)**, same selector as Ramas: the typed branch is the **base** ref (any trailing `-mx` is stripped by `pubBaseRef`), and the variant decides which refs publish — `rb/x`, `rb/x-mx`, or both. `createReleases` takes ONE ref, so "Ambas" is **two sequential calls** whose results are concatenated with their `ref` attached (a failing project has no tag, so the ref is the only thing telling the two rows apart). The status-poll map is keyed `id@tag`, not `id` — with both variants the same project publishes two tags and one would overwrite the other.
