...
Which sidebar sections are visible. **Single source of truth = `MENU_SECTIONS` in `core.js`** (key → label/icon/`navId`/`gitlabOnly`, in menu order); `applyMenuVisibility()` (called in `boot()`) hides each section's `navId` element when it's GitLab-only-on-GitHub OR not in `config.sections`. Each `navId` is a wrapper `<div>` around the section's buckets — except `prs`, whose `navId` (`#nav-prs-section`) is the lone "Merge requests" bucket itself (`data-bucket="open"`, all open MRs + count).

Sections are grouped visually in `index.html` under `.nav-group[data-tone]` headers — Código (prs, historial, historico) · Trabajo local · Planificación (milestones, propuestas) · Entrega (releases, entornos) · Soporte. A group is a pure visual layer: it is not a config key, `applyMenuVisibility()` hides its header when no section inside is visible, and `data-tone` tints its icons + the active entry. The active repo selector (`#repo-select`) lives in the topbar, hidden with a single repo; Ajustes only edits the repo *list*. The old Mías/Para revisar/Borradores buckets are gone; a stale `config.lastBucket` from them falls back to `open` in `boot()`. Digit shortcuts: `1` MRs · `2` Fusionadas · `3` Cerradas.
