...
- `src/config.js` — config.json in `app.getPath("userData")` (`provider`, `gitlabBaseUrl`, repos[], pollSeconds, optional manual token, `cherryPick` `{prefix, branches[], siblingMx}`, `milestones` `{group, statusLabels[], doneLabels[]}`, `releases` `{sourceBranch, branchPrefix, projects[{id,name,note?}]}`). `config:set` in main.js is a strict whitelist — new config keys must be validated/added there or they're silently dropped. The deep-merged `cherryPick`, `milestones` and `releases` keys also need deep-merge lines in `config.load()`.
...
- **Releases view** (GitLab-only, `state.view==="releases"`): a top-level view (like history/milestones) that **generates release branches** `${branchPrefix}<version>` (default `rb/`) across a **configurable set of projects** — a 1:1 replication of the legacy `auto-rb-branches.py` (script attached on the originating issue OPE-18). Config `releases` = `{sourceBranch (default "development"), branchPrefix (default "rb/"), projects:[{id,name,note?}]}`; the default `projects` are the 8 from the script (id = GitLab **numeric id or path**, either works as the API project identifier — `proj()` just URL-encodes it). UI: version input (placeholder `MMYYYY-mx`, **not** prefilled — branch creation is a write, so the value must be deliberate; live `rb/…` preview + same `BRANCH_RE` validation as branches), source-branch input (default `development`), and a checkbox list of the configured projects (**all selected by default**; the user **deselects** ones to skip). Per-project `note` (e.g. Ouicare "Cambiar AppDate antes de crear la rama") shows as a ⚠️ reminder on the row and in the confirm modal. **Never auto-fires**: a confirm modal lists branch + source + target projects before creating. `generateReleaseBranches({projects,version,sourceBranch})` POSTs `repository/branches {branch,ref}` per project, **sequentially and non-atomically** (one failure doesn't stop the rest), returning `{branch, ref, results:[{id,name,ok,error?,webUrl?}]}`; the renderer shows ✓ created (links to the branch) / ✕ error per row. **Security**: the legacy script hardcoded a GitLab token — **never replicate that**; the token resolves through `resolveToken` like everywhere else, and `releases:generate` in main.js **intersects the renderer's requested ids with the configured set** so the renderer can never create branches on arbitrary projects. GitHub side: throwing stubs (`releaseDefaults`/`generateReleaseBranches`) for parity; nav entry hidden when `provider!=="gitlab"`. The view is **repo-agnostic** (project-set scoped, like milestones is group-scoped): switching the repo selector doesn't touch it. Selftest route `--selftest-route=releases`.
