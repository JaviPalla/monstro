...
- `src/github.js` / `src/gitlab.js` — the two provider implementations. **Same public interface**; both normalize to the **GitHub data shape** the renderer consumes. The renderer must stay provider-agnostic. (`cherryPick` is GitLab-only — the GitHub side is a throwing stub kept only for interface parity; the feature is gated to `provider==="gitlab"` in the renderer.)
- `src/config.js` — config.json in `app.getPath("userData")` (`provider`, `gitlabBaseUrl`, repos[], pollSeconds, optional manual token, `cherryPick` `{prefix, branches[], siblingMx}`). `config:set` in main.js is a strict whitelist — new config keys must be validated/added there or they're silently dropped. The deep-merged `cherryPick` key also needs a deep-merge line in `config.load()`.
